...
export function Navigation({ className, onLinkClick }) {
  ...
  return (
    <nav className={clsx('text-base lg:text-sm max-w-full', className)}>
      <ul role="list" className="space-y-9 max-w-full">
        {navigation[locale].map((section) => (
          <li key={section.title} className="max-w-full">
            <h2 className="font-display font-medium text-slate-900 dark:text-white break-words">
              {section.title}
            </h2>
            <ul
              role="list"
              className="mt-2 space-y-2 border-l-2 border-slate-100 lg:mt-4 lg:space-y-4 lg:border-slate-200 dark:border-slate-200/50 max-w-full"
            >
              {section.links.map((link) => (
                <li key={link.href} className="relative max-w-full">
                  <Link
                    href={getLocalizedUrl(link.href, locale)}
                    onClick={onLinkClick}
                    className={clsx(
                      'block w-full pl-3.5 pr-4 before:pointer-events-none before:absolute before:-left-1 before:top-1/2 before:h-1.5 before:w-1.5 before:-translate-y-1/2 before:rounded-full break-words overflow-wrap-anywhere',
