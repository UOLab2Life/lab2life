      ...
      <figure className="relative p-10">
        <figcaption className="mt-6">
          <div className="space-y-1">
            <p className="text-3xl/7 font-inter-semibold text-white">{name}</p>
            <p className="text-lg/6 font-inter-semibold text-[#b184e9]">{position}</p>
            <p className="text-sm/5 font-inter-semibold text-white leading-tight">{education}</p>
          </div>
          
          <div className="mt-4 flex gap-3">
            <a 
              href={`mailto:${email}`}
              className="p-2 bg-white/10 rounded-full hover:bg-[#003e3e] transition-colors"
...
export function MembersCarousel() {
  ...
  const handleMemberClick = useCallback((index) => {
    scrollTo(index)
